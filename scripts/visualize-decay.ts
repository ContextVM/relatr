...
import { DecayProfiles, getDecayProfile, getAllDecayProfiles, getDecayProfileMetadata, recommendDecayProfile } from '../src/distance/DecayProfiles';
 ...
 */
function generateVisualizationData(options: VisualizationOptions): DecayVisualizationData[] {
    const profiles = options.profiles || ['DEFAULT', 'CONSERVATIVE', 'PROGRESSIVE', 'BALANCED', 'STRICT', 'EXTENDED'];
    ...
    for (const profileName of profiles) {
        ...
        const metadata = getDecayProfileMetadata(profileName as keyof typeof DecayProfiles);
...
function outputAsTable(data: DecayVisualizationData[]): void {
    ...
    const maxDistance = Math.max(...data.flatMap(d => d.points.map(p => p[0])));
...
function compareProfiles(): void {
    ...
    // Header
    console.log('Profile'.padEnd(12) + ' | α  | Zero | ' + testDistances.map(d => `D${d}`).join(' | '));
    console.log('-'.repeat(12) + '-|----|------|-' + testDistances.map(() => '----').join('-+'));
    ...
    for (const [profileName, config] of Object.entries(allProfiles)) {
        ...
        let row = profileName.padEnd(12) + ` | ${config.decayFactor.toFixed(2).padStart(2)} | ${zeroThreshold.toString().padStart(4)} | `;
    ...
    for (const profileName of Object.keys(allProfiles)) {
        ...
        console.log(`  Zero Weight Threshold: ${metadata.zeroWeightThreshold} hops`);
...
function showRecommendations(): void {
    ...
    const useCases = ['social', 'professional', 'security', 'exploration'] as const;
    
    for (const useCase of useCases) {
        const recommendation = recommendDecayProfile(useCase);
        const metadata = getDecayProfileMetadata(recommendation);
        
        console.log(`${useCase.charAt(0).toUpperCase() + useCase.slice(1)} networks:`);
...
function showUsageExamples(): void {
    ...
    console.log('const strictNormalizer = new DistanceNormalizer(DecayProfiles.STRICT);');
    ...
    console.log('const distances = new Map([["alice", 1], ["bob", 3], ["charlie", 5]]);');
    ...
    console.log('console.log(`Zero weight at: ${stats.zeroWeightThreshold} hops`);');
    console.log();
}
...
Examples:
  ...
  bun run visualize-decay.ts --recommendations
    ...
    if (data.length > 0) {
        console.log(`=== DECAY CURVES (Max Distance: ${options.maxDistance || 20}) ===\n`);
