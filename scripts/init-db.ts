...
function verifySchema(db: Database): boolean {
    ...
    try {
        ...
        const metricCount = db.query("SELECT COUNT(*) as count FROM metric_definitions").get() as { count: number };
        console.log(`Metric definitions: ${metricCount.count}`);
        
        // Verify configuration was inserted
        const configCount = db.query("SELECT COUNT(*) as count FROM configuration").get() as { count: number };
...
function testDatabaseOperations(db: Database): boolean {
    ...
    try {
        ...
        console.log(`Found ${metrics.length} active metrics:`, metrics.map(m => (m as any).metric_name));
