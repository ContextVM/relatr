...
export class MetricsRepository {
  ...
  /**
   * Save metadata for a specific metric in the profile_metrics table
   * @param pubkey The public key
   * @param metricKey The metric key to store metadata for
   * @param meta Metadata object to store (will be JSON stringified)
   */
  async saveMeta(
    pubkey: string,
    metricKey: string,
    ...
    try {
      return await executeWithRetry(async () => {
        return await dbWriteQueue.runExclusive(async () => {
          ...
          // Update existing metric entry with metadata
          await this.writeConnection.run(
            "UPDATE profile_metrics SET meta = $1 WHERE pubkey = $2 AND metric_key = $3",
            { 1: metaJson, 2: pubkey, 3: metricKey },
          );
        });
      });
    } catch (error) {
      logger.warn(
        `Failed to save meta for ${pubkey}:${metricKey} after retries:`,
        error instanceof Error ? error.message : String(error),
      );
      throw new DatabaseError(
        `Failed to save meta for ${pubkey}:${metricKey}: ${error instanceof Error ? error.message : String(error)}`,
        "META_SAVE",
      );
    }
  }
  ...
  /**
   * Get metadata for a specific metric from the profile_metrics table
   * @param pubkey The public key
   * @param metricKey The metric key to retrieve metadata for
  ...
  async getMeta(
    ...
    try {
      return await executeWithRetry(async () => {
        const now = Math.floor(Date.now() / 1000);
        const result = await this.readConnection.run(
          "SELECT meta FROM profile_metrics WHERE pubkey = $1 AND metric_key = $2 AND expires_at > $3 LIMIT 1",
        ...
        const rowArray = rows[0] as unknown[];
        const metaText = rowArray[0] as string;
        ...
        try {
          ...
          logger.warn(
            `Failed to parse meta JSON for ${pubkey}:${metricKey}:`,
      ...
      logger.warn(
        `Failed to get meta for ${pubkey}:${metricKey} after retries:`,
        error instanceof Error ? error.message : String(error),
      );
      return null;
    }
