...
/**
 * Generic cache implementation using in-memory SQLite database
...
export class SimpleCache<T> {
    ...
    /**
     * Create a new SimpleCache instance with in-memory database
     * @param tableName - Name of the cache table
     * @param ttlSeconds - Time to live for cache entries in seconds
     */
    constructor(tableName: string, ttlSeconds: number) {
        // Create in-memory database for cache to avoid conflicts with main database
    ...
    private prepareStatements(): void {
        try {
            ...
            this.deleteAllQuery = this.db.query(`
    ...
    async get(key: CacheKey): Promise<T | null> {
        try {
            ...
            const result = this.getQuery.get(keyStr, now) as { value: string } | undefined;
    ...
    async getStats(): Promise<{
        ...
        try {
            ...
            // Count total entries
            const totalQuery = this.db.query(`SELECT COUNT(*) as count FROM ${this.tableName}`);
     ...
     */
    async setWithTTL(key: CacheKey, value: T, customTtlSeconds: number): Promise<void> {
        try {
            const keyStr = this.keyToString(key);
            const now = Math.floor(Date.now() / 1000);
