...
export class RelatrService implements IRelatrService {
  ...
  async calculateTrustScore(
    params: CalculateTrustScoreParams,
    enableLazyRefresh: boolean = true,
  ): Promise<TrustScore> {
    ...
    if (!decodedTargetPubkey) {
      throw new ValidationError(
        "Invalid target pubkey format. Must be hex, npub, or nprofile",
        "targetPubkey",
      );
    }
    ...
    const result = await this.calculateTrustScoresBatch(
      ...
      enableLazyRefresh,
    ).then((results) => results.get(decodedTargetPubkey));

    if (!result) {
      throw new RelatrError(
        "Trust calculation failed: missing trust score result",
        "CALCULATE_TRUST",
      );
    }
   ...
   */
  async calculateTrustScoresBatch(
    params: {
      sourcePubkey?: string;
    ...
    if (!this.initialized)
      throw new RelatrError("Not initialized", "NOT_INITIALIZED");

    const { sourcePubkey, targetPubkeys } = params;

    if (
      !targetPubkeys ||
      !Array.isArray(targetPubkeys) ||
    ...
    if (!decodedSourcePubkey) {
      throw new ValidationError(
        "Invalid source pubkey format. Must be hex, npub, or nprofile",
        "sourcePubkey",
      );
    }
    ...
    try {
      ...
      for (const targetHex of decodedTargetPubkeys) {
        ...
        const enrichedTrustScore = this.enrichTrustScoreWithDescriptions(
          trustScore,
          metricDescriptions,
        );
      ...
      if (enableLazyRefresh && this.taService) {
        ...
        for (
          let i = 0;
          i < decodedTargetPubkeys.length;
          i += CONCURRENCY_LIMIT
        ) {
          const batchStart = i;
          const batchEnd = Math.min(
            i + CONCURRENCY_LIMIT,
            decodedTargetPubkeys.length,
          );
      ...
      throw new RelatrError(
        `Trust calculation failed: ${error instanceof Error ? error.message : String(error)}`,
        "CALCULATE_TRUST",
      );
    }
  }
  ...
  async getStats(): Promise<StatsResult> {
    ...
    try {
      ...
      let socialGraphStats: StatsResult["socialGraph"]["stats"] = {
        users: 0,
        follows: 0,
      };
      let rootPubkey = "";
