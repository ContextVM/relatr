...
import { ValidationError, SocialGraphError } from '../types';
...
export class MetricsValidator {
    ...
    async validateAll(pubkey: string, sourcePubkey?: string): Promise<ProfileMetrics> {
        ...
        try {
            ...
            const [
                ...
                sourcePubkey ? this.validateReciprocity(sourcePubkey, pubkey) : Promise.resolve(0.0)
            ]);
     ...
     * @returns Validation score (0.0 or 1.0)
     */
    private async validateReciprocity(sourcePubkey: string, targetPubkey: string): Promise<number> {
        if (!sourcePubkey || !targetPubkey || 
            typeof sourcePubkey !== 'string' || typeof targetPubkey !== 'string') {
            return 0.0;
        }
        ...
        try {
            ...
            const sourceFollowsTarget = this.graphManager.doesFollow(sourcePubkey, targetPubkey);
            const targetFollowsSource = this.graphManager.doesFollow(targetPubkey, sourcePubkey);
